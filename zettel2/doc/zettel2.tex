...
\begin{equation}
 \zen_{\text{max}} = \arccos{\left( \frac{1}{1.15} \right)} = \ang{29.592}
\end{equation}

According to Exercise B on Sheet 1 the zenith angle \zen is connected to the 
declination \DEC of the object, the geographical latitude \LAT of the 
observer, and the hour angle \HA by the formula

\begin{equation}
    \sin{ \left( \ang{90}-\zen \right)} = \cos{\left( \zen \right)} = 
    \cos{ \left( \LAT \right)} \cos{ \left( \DEC \right)} 
    \cos{ \left( \HA \right)} + \sin{ \left( \LAT \right)} 
    \sin{ \left( \DEC \right)}
\end{equation}

Since $\LAT$ and $\DEC$ are known, the condition to $\zen$ can be used to 
compute the corresponsing hour angle: 

\begin{equation}
    \HA = \arccos{\left( \frac{\cos{\left( \zen \right)} - 
    \sin{\left( \LAT \right)} \sin{\left( \DEC \right)}}
    {\cos{ \left( \LAT \right) } \cos{ \left( \DEC \right) }} \right)}
\end{equation}

With $\zen = \ang{29.592}$, $\DEC = \ang{-0.111}$, and $\LAT = \ang{19.825}$ 
it follows 

\begin{equation}
 \HA = \ang{22.324} = \SI{1}{^\hour} \SI{29}{^\minute} \SI{17.8}{^\second}
\end{equation}
...
\begin{equation}
 \Delta t = 2 \HA = \SI{2}{^\hour} \SI{58}{^\minute} \SI{35.6}{^\second}
\end{equation}
...
zenith distance \zen:
\begin{align}
    X &= \frac{\SI{1}{airmass}}{\cos(\zen)} \;.
\end{align}
As $\zen = \ang{90} - \hei$ we find that $\cos(\zen) = \cos(\ang{90} - \hei) = 
\sin(\hei)$, which is given via the hour angle $HA$, the latitude of the 
observer \LAT and the declination \DEC of the star:
\begin{align}
    \cos(\zen) = \sin(\hei) &= \cos(\LAT)\cos(\DEC)\cos(\HA) + \sin(\LAT)\sin(\DEC) \;.
\end{align}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{pic/umv}
\caption{\emph{Left}: Dependence of color on the hour angle. \emph{Right}: 
         The dependence of X on the hour angle. At $\HA = 0$ we observe 1 
         airmass.}
\label{fig:X}
\end{figure}
